...
6. **Write.** Copy `f-000-feature-template.md` to `f-001-<slug>.md`,
   ...
   technical-writer/markdown-author chat mode or extension if available) to fill in
   the sections covered by the answers, set status to `Draft` or `Review Pending`
   as appropriate, and set the date fields. Leave uncovered sections as
   placeholders.
