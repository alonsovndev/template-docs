...
6. **Write.** Copy `adr-template.md` to `adr-0XX-<slug>.md` (numbering continues
   ...
   (use a technical-writer/markdown-author chat mode or extension if available) to
   fill in the sections covered by the answers, set status to `Proposed` or
   `Accepted`, and set the date fields. Leave uncovered sections as placeholders.
   Then add the row to the Decision Log table in
