...
4. **Ask, capped at 10 for this pass.** Use as few as the subfolder needs — often
   ...
   ≤4. If a prior pass already answered something relevant (e.g. tech stack chosen
   in `core` also answers a `database` question), reuse it instead of asking
   again.

5. **Write.** Use `technical-writer` and `markdown-author` skills for prose;
   ...
   fields and the "Source References" section if it should now point to specific
   ADRs or feature docs. Leave uncovered sections as placeholders.
