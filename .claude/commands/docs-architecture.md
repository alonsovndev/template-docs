...
4. **Ask, capped at 10 for this pass.** Use as few as the subfolder needs — often
   ...
   ≤4. If an earlier pass — including an earlier invocation of this command —
   already answered something relevant (e.g. tech stack chosen in `core` also
   answers a `database` question), read it back from the written docs and ADRs
