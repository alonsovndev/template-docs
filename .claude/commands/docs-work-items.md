...
5. **Ask, capped at 10.** Use as few as actually needed — often the epic-grouping
   strategy and project key/components cover most of it. Use `AskUserQuestion`,
   split across multiple calls of ≤4.
...
6. **Write.** Create a new `work-items/` folder at the repo root (sibling to
   ...
   stories are grouped by role. Use the `technical-writer` and `markdown-author`
   skills for phrasing and formatting.
