...
5. **Ask, capped at 10 total per command run** (across all features requested this
   ...
   open items. Use `AskUserQuestion`, split across multiple calls of ≤4 questions
   each. Prefer breadth (one key question per feature) over depth on a single
   feature if multiple features were requested.
...
6. **Write.** Copy `f-000-feature-template.md` to `f-001-<slug>.md`,
   ...
   file). Use `technical-writer` and `markdown-author` skills to fill in the
